            ...
            <form onSubmit={handleUpdateProfile} className="space-y-8 md:space-y-16">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12">
                    <div className="space-y-4">
                        <label className="text-2xs font-black tracking-[0.4em] uppercase text-black">Legal Identifier</label>
                        <input 
                          type="text" 
                          value={form.username}
                          onChange={(e) => setForm({...form, username: e.target.value})}
                          placeholder="FULL NAME SPEC" 
                          className="w-full bg-transparent border-b border-black/10 py-4 text-xs font-bold tracking-widest focus:outline-none focus:border-black uppercase transition-colors" 
                        />
                    </div>
                    <div className="space-y-4">
                        <label className="text-2xs font-black tracking-[0.4em] uppercase text-black/30">Primary Email [Static]</label>
                        <input 
                          type="email" 
                          value={form.email}
                          disabled 
                          className="w-full bg-transparent border-b border-black/5 py-4 text-xs font-bold tracking-widest text-black/20 uppercase cursor-not-allowed" 
                        />
                    </div>
                </div>

                <div className="space-y-4">
                    <label className="text-2xs font-black tracking-[0.4em] uppercase text-black">Communication Node</label>
                    <input 
                      type="tel" 
                ...
                <div className="space-y-4">
                    <label className="text-2xs font-black tracking-[0.4em] uppercase text-black">Fulfillment Address</label>
                    <textarea 
                      value={form.address}
                      onChange={(e) => setForm({...form, address: e.target.value})}
                      placeholder="ENTER FULL DELIVERY SPECIFICATION..." 
                      rows={4} 
                      className="w-full bg-transparent border-b border-black/10 py-4 text-xs font-bold tracking-widest focus:outline-none focus:border-black uppercase transition-colors resize-none" 
                    />
                </div>
                ...
                <div className="pt-8">
                    <button 
                       disabled={loading}
                       className="flex items-center gap-8 bg-black text-white px-20 py-8 text-xs font-black uppercase tracking-[0.3em] hover:bg-brand-teal transition-all duration-700 disabled:opacity-50"
                    >
          ...
          <div className="lg:col-span-4 space-y-12">
             <div className="p-12 bg-[#FAFAFA] border border-black/5 space-y-8">
                <div className="flex items-center gap-4 text-brand-teal">
                   ...
                   <p className="text-xs font-black uppercase tracking-widest">Health Sync Active</p>
                </div>
             ...
             <div className="p-12 border border-black/5 space-y-8">
                <div className="flex items-center gap-4 text-black">
                   ...
                   <p className="text-xs font-black uppercase tracking-widest">Security Protocol</p>
                </div>
                <p className="text-xs font-bold uppercase leading-relaxed tracking-widest text-black/40">
                ...
                <button className="text-2xs font-black uppercase tracking-[0.2em] border-b border-black pb-1 hover:text-brand-teal hover:border-brand-teal transition-all">Audit Security Log</button>
             </div>
          </div>
